...
OCL_SDF::OCL_SDF(size_t width, size_t height, bool printInfo)
  : width(width), height(height), imageSize(width * height), sdfPixels(new u8[width * height]){
...
void OCL_SDF::updateCirclesBuffer(const std::vector<Circle2D>& circles) {
  ...
  for (size_t i = 0; i < numCircles; i++) {
    ...
    const Circle2D& circle = circles[i];
    float radius = circle.getRadius();
    vec2 pos = circle.getPosition();
    clPos = {{pos.x, pos.y}};

    hostCircles[i] = {clPos, radius};
  }

  [[maybe_unused]]
  cl_int hostCopyResult = clEnqueueWriteBuffer(commandQueue, gpuCircles, CL_TRUE, 0, sizeof(CircleCL) * numCircles, hostCircles, 0, nullptr, nullptr);
...
void OCL_SDF::updateRectsBuffer(const std::vector<Rectangle2D>& rects) {
  ...
  for (size_t i = 0; i < numRects; i++) {
    ...
    const Rectangle2D& rect = rects[i];
    vec2 sizeFromCenter = rect.getSize() * 0.5f;
    vec2 centerGlobal = rect.getPosition();

    clCenterGlobal = {{centerGlobal.x, centerGlobal.y}};
    clSizeFromCenter = {{sizeFromCenter.x, sizeFromCenter.y}};

    hostRectangles[i] = {clCenterGlobal, clSizeFromCenter};
  }

  [[maybe_unused]]
  cl_int hostCopyResult = clEnqueueWriteBuffer(commandQueue, gpuRectangles, CL_TRUE, 0, sizeof(RectangleCL) * numRects, hostRectangles, 0, nullptr, nullptr);
  assert(hostCopyResult == CL_SUCCESS);
}
